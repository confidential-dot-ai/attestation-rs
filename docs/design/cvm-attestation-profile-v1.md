...
Status: draft for review. Owner: attestation DRI. The normative specification is now the standalone [CVM Attestation v1 standard](../standard/cvm-attestation-v1.md): where sections 3 to 7, 10 and 14 of this document and the standard differ, the standard governs, and the conformance corpus cites the standard's sections. This document remains the design record: the Rust surface, the migration, the decisions and the implementation plan. Companion documents: "Standardizing Attestation" and "SEV-SNP Measurement Registers" (Notion, Confidential AI / Docs). This document is the library-side design those two ask for, and it changes both of them; section 11 lists exactly what.
...
10. Conformance (section 14): the corpus, the Rust runner that generates and checks it, and the wasm entry's run over the same corpus; attestation-go and c8s-verify-js run the corpus with their own runners, in their own repositories, once they appraise the profile (step 6). Every later change to a decision in sections 4 to 7 lands with its case.
...
The corpus is data. Every implementation runs it with its own runner, in its own repository, and pins it by the attestation-rs commit or tag it was taken from. attestation-rs holds the reference runner (`crates/attestation/tests/conformance.rs`) and runs the corpus through the wasm entry as well (`crates/attestation-wasm/tests/conformance.rs`).
...
For `expect.refusal`, the runner maps the implementation's error to one code of section 14.4 and compares codes. An implementation documents its mapping: the reference implementation's is `AttestationError::refusal_code`, and the wasm build throws an `Error` whose `code` is that code. An error the mapping does not cover is no decision, reported as an error, never as a refusal. A refusal with another code is a different decision and fails the case, as does an appraisal where a refusal was expected or the reverse.
...
Moved to the [standard](../standard/cvm-attestation-v1.md), Appendix A.

## Appendix B. Test vectors

Moved to the [standard](../standard/cvm-attestation-v1.md), Appendix B, with the machine-readable vectors in `docs/standard/vectors/cvm_profile_vectors.json` and their independent generator in `crates/attestation/tests/vector_generator.rs`.

## Appendix C. CDDL module

Moved to the [standard](../standard/cvm-attestation-v1.md), Appendix C; the module is `schemas/cvm-profile-v1.cddl`.
