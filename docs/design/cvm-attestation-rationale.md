...
| `ear_all_submods_bound` | Adapted from a separate [TDX/confidential-GPU profile draft](https://datatracker.ietf.org/doc/html/draft-kykdxy-rats-tdx-cgpu-ear-profile-02), not core EAR. The claim takes that draft's text type (item 2). |
...
2. **Binding extension compatibility.** The [TDX/GPU draft](https://datatracker.ietf.org/doc/html/draft-kykdxy-rats-tdx-cgpu-ear-profile-02#section-3) defines `ear_all_submods_bound` as strings `"true"`, `"false"`, or `"unknown"`; this branch emits a boolean. Agree on the type and proof semantics, or give the narrower CVM claim a distinct name. Do not claim consumers need no translation yet. *Resolved:* the verifier emits the draft's text, `"true"` or `"false"` (never `"unknown"`, since every binding is checked); a failed binding is a refusal, so `"false"` appears only for a device whose signed nonce match is false under a policy that tolerates it (section 5.3).
