...
- TDX: the attestation key type and the QE vendor identifier are not checked; TCB level matching follows the TCB Info's order in two passes, departs from Intel's algorithm for an all-zero TEE TCB SVN and when `TEE_TCB_SVN[1]` is not zero, and does not check the TDX module identity; the TD attribute mask refuses bits the ABI specification permits (`ICSSD`, `SERVTD_EXT`, `RESERVED_P`, `LASS`, `TPA`) and the debug refusal, `cvm_policy.debug`, `dbgstat`, `configuration` and `runtime-opaque` reflect bit 0 alone, so the profiling bits pass as non-debug when reserved-attribute checking is waived; the binding of a TCB Info to the PCK certificate's FMSPC and PCE identifier is checked for fetched and inline collateral but not for collateral a caller supplies directly; a TCB Info or QE Identity without `nextUpdate` is accepted; the QE Identity's TCB status is not combined with the platform's, and the advisories are the platform level's alone; the TCB signing certificate's role is not checked.
