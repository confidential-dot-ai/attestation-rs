...
| 9.7, 12.6, 13.5 | the NRAS exchange: token signatures, issuer, claims version, `submods` digests, the device nonce, the device architecture, device claims, `ear_nvidia_evidence`, the device gates, and `ear_all_submods_bound` `"false"` | a recording of GPU and NVSwitch evidence with its NRAS exchange; the device submodule's shape, the architecture allowlist, the device bound and `gpu.required` have cases |
| 9.4.1 | the HCL report's type names the TEE of the `cpu` submodule; the hash type is 1; the variable data is exactly its declared size | cases that change the recorded HCL report, which the TPM quote does not sign |
| 9.4.4 | a `tpm2-event-log` on the `vtpm` submodule replays | an Azure recording that includes the vTPM event log |
| 9.1.2, 9.1.3, 9.1.4 | report version 6; Turin models 0x12 to 0x1F; a VLEK chain; `SIGNATURE_ALGO`; the key-selection bits; the signature over the received bytes, with the upper bytes of `R` and `S` zero; the ASK or ASVK serial against the CRL; the ARK and ASK windows; an all-zero `CHIP_ID` matching no allowlist entry | a VLEK recording, and the reference implementation corrections listed in section 18 |
| 9.2.3, 9.2.5 | the attestation key type and QE vendor ID; the TDX module identity; Intel's TCB level matching; the TCB Info's FMSPC and PCE binding for collateral supplied directly; a TCB Info without `nextUpdate`; the QE status combined with the platform status; the TCB signing certificate's role; the TD attribute groups of section 9.2.5 (profiling counted as debug, `RESERVED_P` ignored, `ICSSD`, `SERVTD_EXT`, `LASS` and `TPA` permitted) | recordings and collateral that exercise each, and the reference implementation corrections listed in section 18 |
