//! The CVM attestation profile v1: `docs/design/cvm-attestation-profile-v1.md`.
//!
//! Types here are the profile one to one. Evidence and policy parse fail-closed:
//! every structural rule in sections 4 and 7 is enforced by `validate` before a
...
/// `__cmwc_t` of `cvm_endorsements` (section 4.6).
...
/// Media types of endorsement entries (section 4.6).
...
/// `eat_nonce` bounds (section 4.5).
pub const NONCE_MIN: usize = 16;
pub const NONCE_MAX: usize = 64;

/// Section 4.7: a submodule's register array must not exceed this. Sixteen SNP
